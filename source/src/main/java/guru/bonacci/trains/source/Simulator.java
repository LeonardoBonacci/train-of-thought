...
public class Simulator {
	...
	final static double START_LAT = 33.00;
	final static double START_LON = -115.00;
	final static double END_LAT = 33.10;
	final static double END_LON = -115.10;

	private List<Station> stations = Collections.unmodifiableList(
			Arrays.asList(Station.builder().id(0).name("Johnsonville Station").lat(33.01).lon(-115.01).build(),
	...
	List<Train> trains = Lists.newArrayList(newTrain());

    public Train newTrain() {
        return Train.builder().id(UUID.randomUUID().toString())
					.route(66)
					.name("JVL-WEL")
					.lat(START_LAT)
					.lon(START_LON)
	...
	public Flowable<KafkaMessage<String, String>> trainEvents() {
		return Flowable.interval(1000, TimeUnit.MILLISECONDS).onBackpressureDrop().map(tick -> {
			...
			if (train.lat > END_LAT || train.lon < train.lon)
				trains.set(0, newTrain()); //FIXME for now
			
			String payload = "{ \"id\" : \"" + train.id + "\"" + ", \"route\" : " + train.route + ", \"name\" : \""
					+ train.name + "\"" + ", \"moment\" : \"" + Instant.now() + "\"" + ", \"lat\" : " + train.lat
					+ ", \"lon\" : " + train.lon + "}";
